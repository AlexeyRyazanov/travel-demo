...
import convertMaskToPlaceholder from './convert-mask-to-placeholder';
import getSafeRawValue from '../input/input-get-safe-raw-value';
import MaskedInputType from '../../typings/masked-input';
import processCaretTraps from './process-caret-traps';
import safeSetSelection from './safe-set-selection';

const emptyString = '';

// mask = providedMask(safeRawValue, { currentCaretPosition, previousConformedValue, placeholderChar });

export default function createTextMaskInputElement(config: MaskedInputType) {
   ...
   return {
      ...
      update(rawValue: any, {
         ...
         const finalConformedValue = typeof pipe === 'function' ? pipeResults.value : conformedValue;
